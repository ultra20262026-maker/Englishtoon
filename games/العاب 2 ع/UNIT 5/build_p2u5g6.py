...
bg_path = get_latest_image("hologram_map_bg", brain_dir)
pin_path = get_latest_image("hologram_pin_prop", brain_dir)
...
<head>
    ...
    <title>الدرس السادس - الخريطة الهولوغرامية</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        .spinner {{
            ...
            border-top-color: #A855F7; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 20px;
        }}
        ...
        .right-col {{
            background: rgba(15, 23, 42, 0.5); border: 2px solid rgba(168, 85, 247, 0.3); border-radius: 1rem;
            height: 80vh; position: relative; box-shadow: inset 0 0 40px rgba(168, 85, 247, 0.1);
        ...
        .game-title {{ font-size: 2.2rem; font-weight: 900; color: #A855F7; margin: 0; text-shadow: 0 0 10px rgba(168,85,247,0.5); }}
        ...
        .question-box {{
            background: rgba(15, 23, 42, 0.85); border: 2px solid rgba(168, 85, 247, 0.3); border-radius: 1.5rem;
        ...
        .answer-btn {{
            background: rgba(255, 255, 255, 0.05); border: 2px solid rgba(168, 85, 247, 0.4); color: #FFFFFF;
        ...
        .answer-btn:hover {{ background: rgba(168, 85, 247, 0.2); border-color: #D8B4FE; transform: translateY(-3px); }}
        ...
        .start-btn {{
            background: #A855F7; border: none; color: #0F172A; padding: 1.5rem 3rem; font-size: 2rem; font-weight: 900;
            border-radius: 1rem; cursor: pointer; margin-top: 3rem; align-self: center; box-shadow: 0 10px 20px rgba(168,85,247,0.4);
        }}

        /* Hologram Map Scene */
        .map-container {{
            position: relative; width: 300px; height: 300px;
            border-radius: 50%; border: 4px solid rgba(168, 85, 247, 0.2);
            background: radial-gradient(circle, rgba(168, 85, 247, 0.1) 0%, transparent 70%);
            transform: rotateX(60deg); transform-style: preserve-3d;
            box-shadow: 0 20px 50px rgba(168, 85, 247, 0.3), inset 0 0 50px rgba(168,85,247,0.2);
        }}
        ...
        .map-grid {{
            ...
            animation: rotate-map 20s linear infinite;
        }}
        ...
        .pin {{
            ...
            background-image: url('{pin_b64}'); background-size: contain; background-repeat: no-repeat;
            transform: rotateX(-60deg) translateY(-20px); opacity: 0; transition: opacity 0.5s ease;
            filter: drop-shadow(0 0 10px #D8B4FE);
        }}

        /* Pre-defined pin positions */
        .pin:nth-child(1) {{ top: 20%; left: 30%; }}
        .pin:nth-child(2) {{ top: 40%; left: 60%; }}
        .pin:nth-child(3) {{ top: 70%; left: 40%; }}
        .pin:nth-child(4) {{ top: 30%; left: 80%; }}
        .pin:nth-child(5) {{ top: 80%; left: 70%; }}
        .pin:nth-child(6) {{ top: 50%; left: 20%; }}
        .pin:nth-child(7) {{ top: 15%; left: 60%; }}
        .pin:nth-child(8) {{ top: 60%; left: 80%; }}
        .pin:nth-child(9) {{ top: 85%; left: 30%; }}
        .pin:nth-child(10) {{ top: 45%; left: 45%; }}

        .pin.active {{ opacity: 1; animation: bounce-pin 1s ease infinite alternate; }}

        @keyframes bounce-pin {{
            0% {{ transform: rotateX(-60deg) translateY(-20px); filter: drop-shadow(0 0 10px #D8B4FE); }}
            100% {{ transform: rotateX(-60deg) translateY(-30px); filter: drop-shadow(0 0 20px #E9D5FF); }}
        }}
        ...
        .alert-box {{ background: #1E293B; padding: 3rem; border-radius: 2rem; text-align: center; border: 2px solid #A855F7; }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">الخريطة الهولوغرامية 🗺️</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">تفعيل الخريطة!</button>
    ...
    <div class="alert-overlay" id="alert-overlay">
        <div class="alert-box">
            ...
            <button class="answer-btn" id="alert-btn" style="background: #A855F7; border: none; color: #0F172A; width: 100%; justify-content: center;" onclick="nextQuestion()"></button>
        </div>
    </div>
    ...
    <script>
        ...
        function playScan() {{
            playTone(800, 1200, 'sine', 0.1, 0.1);
            setTimeout(() => playTone(1200, 1600, 'sine', 0.1, 0.1), 100);
            setTimeout(() => playTone(1600, 2000, 'sine', 0.2, 0.2), 200);
        }}
        ...
        const questions = [
            {{ q: "I wrote a .......... about how to help the poor in our city.", options: ["paragraph", "car", "tree", "game"], correct: 0 }},
            {{ q: "Helping others makes us feel .......... .", options: ["bad", "sad", "happy", "sick"], correct: 2 }},
            {{ q: "You can donate .......... to the charities to buy food.", options: ["stones", "money", "sand", "air"], correct: 1 }},
            {{ q: "Old clothes can be .......... to people who need them.", options: ["eaten", "drunk", "given", "broken"], correct: 2 }},
            {{ q: "A good citizen always .......... the rules of his country.", options: ["breaks", "follows", "forgets", "hates"], correct: 1 }},
            {{ q: "We should .......... the environment and keep it clean.", options: ["damage", "protect", "pollute", "burn"], correct: 1 }},
            {{ q: "Everyone can make a .......... in their community.", options: ["difference", "cake", "noise", "fire"], correct: 0 }},
            {{ q: "They work as .......... in the hospital without money.", options: ["thieves", "volunteers", "babies", "birds"], correct: 1 }},
            {{ q: "My dream is to build a .......... for sick children.", options: ["prison", "cinema", "hospital", "cafe"], correct: 2 }},
            {{ q: "You should be .......... and help your friends.", options: ["cruel", "kind", "angry", "bad"], correct: 1 }}
        ];
        ...
        function updateMap() {{
            const pin = document.getElementById(`pin-${{currentIndex - 1}}`);
            if(pin) pin.classList.add('active');

            if(currentIndex === questions.length) {{
                document.getElementById('map-container').style.boxShadow = "0 20px 80px rgba(168, 85, 247, 0.8), inset 0 0 80px rgba(168,85,247,0.5)";
            }}
        }}
        ...
        function renderQuestion() {{
            ...
            document.getElementById('question-text').innerHTML = `<span style="color: #A855F7;">Q${{currentIndex + 1}}:</span> ${{q.q}}`;
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playWrong();
                handleFail("انتهى الوقت! ⚠️ فقدنا الإشارة!");
            }} else {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! تم تحديد الموقع الجديد 📍");
                    }}, 1000);
                }} else {{
                ...
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ إحداثيات خاطئة!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                ...
                document.getElementById('map-container').style.boxShadow = "0 20px 50px rgba(168, 85, 247, 0.3), inset 0 0 50px rgba(168,85,247,0.2)";
            }}
            renderQuestion();
        }}
        ...
        function showWinScreen() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! لقد فتحت جميع المناطق على الخريطة! 🎉</div>
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">للأسف، فقدنا بيانات الخريطة.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit5_game_lesson6.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 6 for Unit 5 generated successfully.")
