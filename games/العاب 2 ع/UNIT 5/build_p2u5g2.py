...
bg_path = get_latest_image("scientist_lab_bg", brain_dir)
flask_path = get_latest_image("lab_flask_prop", brain_dir)
...
flask_b64 = file_to_b64(flask_path)
...
<head>
    ...
    <title>الدرس الثاني - مختبر العالم</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        .flask {{
            width: 100%; height: 100%; background-image: url('{flask_b64}');
            background-size: contain; background-repeat: no-repeat; background-position: center bottom;
            position: absolute; bottom: 0; z-index: 10;
        }}

        .liquid {{
            position: absolute; bottom: 15%; width: 55%; height: 0%;
            background: linear-gradient(to top, rgba(16,185,129,0.8), rgba(52,211,153,0.9));
            border-radius: 0 0 50px 50px; transition: height 0.8s ease, background 0.5s;
            box-shadow: 0 0 20px rgba(16,185,129,0.5); z-index: 5;
            overflow: hidden;
        }}
        ...
        .bubbles {{
            ...
            background-size: 20px 20px; animation: rise 2s infinite linear; opacity: 0;
        }}
        ...
        .liquid.active .bubbles {{ opacity: 1; }}

        .flask.success .liquid {{
            background: linear-gradient(to top, rgba(139,92,246,0.8), rgba(167,139,250,0.9));
            box-shadow: 0 0 50px rgba(139,92,246,0.8);
            animation: pulse-magic 2s infinite;
        }}

        @keyframes pulse-magic {{
            0% {{ box-shadow: 0 0 30px rgba(139,92,246,0.8); }}
            50% {{ box-shadow: 0 0 80px rgba(167,139,250,1); }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">مختبر العالم 🧪</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">ابدأ التجربة!</button>
    ...
    <script>
        ...
        function playWin() {{
            ...
            osc.frequency.linearRampToValueAtTime(1200, audioCtx.currentTime + 2.0);
        ...
        const questions = [
            {{ q: "The teacher .......... teaches us English is very kind.", options: ["which", "who", "where", "when"], correct: 1 }},
            {{ q: "This is the school .......... I learn.", options: ["who", "which", "where", "what"], correct: 2 }},
            {{ q: "The book .......... I read yesterday was fantastic.", options: ["where", "who", "which", "when"], correct: 2 }},
            {{ q: "Mr. Ahmed is the man .......... helped me.", options: ["who", "which", "where", "whose"], correct: 0 }},
            {{ q: "I love the house .......... my grandfather lives.", options: ["who", "where", "which", "why"], correct: 1 }},
            {{ q: "The car .......... my father bought is very fast.", options: ["who", "where", "which", "what"], correct: 2 }},
            {{ q: "Can you see the boy .......... is wearing a red shirt?", options: ["who", "which", "where", "when"], correct: 0 }},
            {{ q: "Cairo is the city .......... we live.", options: ["who", "where", "which", "that"], correct: 1 }},
            {{ q: "That is the dog .......... barked all night.", options: ["where", "who", "which", "when"], correct: 2 }},
            {{ q: "The doctor .......... works in this hospital is my uncle.", options: ["which", "where", "who", "how"], correct: 2 }}
        ];
        ...
        function updateFlask() {{
            const percentage = ((currentIndex) / questions.length) * 60; // Max height 60%
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playWrong();
                handleFail("انتهى الوقت! ⚠️ فشلت التجربة!");
            }} else {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! تفاعلت المادة 🧪");
                    }}, 1000);
                }} else {{
                ...
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ توقف التفاعل!"), 1000);
            }}
        }}
        ...
        function showWinScreen() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! اكتملت التجربة ونجح التفاعل السحري! 🎉</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
            document.getElementById('flask-container').classList.add('success');
            playWin();
        }}
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">💥</div>
                <div class="question-text" style="color: white; text-align:center;">للأسف، تبخرت المواد وتوقفت التجربة.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit5_game_lesson2.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 2 for Unit 5 generated successfully.")
