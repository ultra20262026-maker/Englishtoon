...
bg_path = get_latest_image("rocket_launchpad_bg", brain_dir)
rocket_path = get_latest_image("spaceship_prop", brain_dir)
...
<head>
    ...
    <title>الدرس الأول - إطلاق الصاروخ</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        /* Rocket Scene */
        .fuel-container {{
            position: absolute; left: 20px; bottom: 20px; width: 30px; height: 300px;
            background: rgba(0,0,0,0.5); border: 2px solid #38BDF8; border-radius: 15px;
            display: flex; flex-direction: column; justify-content: flex-end; overflow: hidden;
            box-shadow: 0 0 15px rgba(56,189,248,0.3); z-index: 20;
        }}
        .fuel-level {{
            width: 100%; height: 0%; background: linear-gradient(to top, #3B82F6, #38BDF8);
            transition: height 0.5s ease;
        }}
        .fuel-label {{
            position: absolute; left: 60px; bottom: 20px; font-weight: bold; color: #38BDF8;
            font-size: 1.2rem; writing-mode: vertical-rl; transform: rotate(180deg);
        }}

        .rocket {{
            width: 150px; height: 300px; background-image: url('{rocket_b64}');
            background-size: contain; background-repeat: no-repeat; background-position: center bottom;
            position: absolute; bottom: 20px; transition: transform 2s cubic-bezier(0.5, 0, 0.5, 1);
            filter: drop-shadow(0 0 10px rgba(56,189,248,0.3)); z-index: 10;
        }}

        .exhaust {{
            position: absolute; bottom: -20px; left: 50%; transform: translateX(-50%);
            width: 40px; height: 60px; background: radial-gradient(ellipse at top, #FDE047, #F97316, transparent);
            filter: blur(5px); opacity: 0; transition: opacity 0.3s; z-index: 5;
            border-radius: 50% 50% 0 0;
        }}
        .rocket.launching .exhaust {{ opacity: 1; animation: flicker 0.1s infinite; }}
        
        @keyframes flicker {{
            0% {{ height: 60px; opacity: 0.8; }}
            50% {{ height: 80px; opacity: 1; }}
            100% {{ height: 70px; opacity: 0.9; }}
        }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">إطلاق الصاروخ 🚀</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">ابدأ تزويد الوقود!</button>
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            ...
            <div class="fuel-label" id="fuel-text">0% FUEL</div>
    ...
    <script>
        ...
        function playFuel() {{
            ...
            setTimeout(() => playTone(800, 1200, 'sine', 0.5, 0.3), 100);
        }}
        ...
        function playLaunch() {{
            ...
            gain.gain.linearRampToValueAtTime(1.0, audioCtx.currentTime + 1);
            gain.gain.linearRampToValueAtTime(0.01, audioCtx.currentTime + 3);
        ...
        const questions = [
            {{ q: "A .......... is a person who works for no money.", options: ["volunteer", "manager", "doctor", "pilot"], correct: 0 }},
            {{ q: "We should .......... money to help poor people.", options: ["steal", "take", "donate", "hide"], correct: 2 }},
            {{ q: "The .......... helps people who are sick or have no home.", options: ["school", "factory", "charity", "bank"], correct: 2 }},
            {{ q: "She was born deaf, so she cannot .......... .", options: ["see", "walk", "hear", "speak"], correct: 2 }},
            {{ q: "A disabled person may need a .......... to move around.", options: ["car", "wheelchair", "bike", "train"], correct: 1 }},
            {{ q: "The street .......... picks up the rubbish every morning.", options: ["cleaner", "driver", "pilot", "engineer"], correct: 0 }},
            {{ q: "A good citizen helps the .......... in his country.", options: ["animals", "community", "space", "weather"], correct: 1 }},
            {{ q: "We can show .......... by helping old people cross the road.", options: ["anger", "sadness", "kindness", "badness"], correct: 2 }},
            {{ q: "It is important to look .......... your grandparents when they are old.", options: ["for", "at", "after", "up"], correct: 2 }},
            {{ q: "He works for a charity that helps .......... children.", options: ["rich", "happy", "poor", "strong"], correct: 2 }}
        ];
        ...
        function updateFuel() {{
            ...
            document.getElementById('fuel-text').innerText = `${{Math.round(percentage)}}% FUEL`;
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playWrong();
                handleFail("انتهى الوقت! ⚠️ تسرب الوقود!");
            }} else {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! تم تزويد الوقود ⛽");
                    }}, 1000);
                }} else {{
                ...
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ انخفض مستوى الطاقة!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                ...
                document.getElementById('fuel-level').style.height = '0%';
                document.getElementById('fuel-text').innerText = '0% FUEL';
            }}
            renderQuestion();
        }}
        ...
        function launchSequence() {{
            document.getElementById('question-box').innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">يتم الانطلاق الآن... 3... 2... 1...</div>
            `;
            ...
            setTimeout(() => {{
                ...
                qBox.innerHTML = `
                    <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">✨</div>
                    <div class="question-text" style="color: white; text-align:center;">تهانينا! انطلق الصاروخ بنجاح! 🎉</div>
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">💥</div>
                <div class="question-text" style="color: white; text-align:center;">للأسف، نفد الوقود وتوقف الإطلاق.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit5_game_lesson1.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 1 for Unit 5 generated successfully.")
