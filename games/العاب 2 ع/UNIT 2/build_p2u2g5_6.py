...
bg_path = get_latest_image("scanner_bg", brain_dir)
...
<head>
    ...
    <title>الدرس الخامس والسادس - المحقق الرقمي</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        .spinner {{
            ...
            border-top-color: #06B6D4; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 20px;
        }}
        ...
        .right-col {{
            background: rgba(15, 23, 42, 0.5); border: 2px solid rgba(6, 182, 212, 0.3); border-radius: 1rem;
            height: 80vh; position: relative; box-shadow: inset 0 0 40px rgba(6, 182, 212, 0.1);
        ...
        .game-title {{ font-size: 2.2rem; font-weight: 900; color: #06B6D4; margin: 0; text-shadow: 0 0 10px rgba(6,182,212,0.5); }}
        ...
        .question-box {{
            background: rgba(15, 23, 42, 0.85); border: 2px solid rgba(6, 182, 212, 0.3); border-radius: 1.5rem;
        ...
        .answer-btn {{
            background: rgba(255, 255, 255, 0.05); border: 2px solid rgba(6, 182, 212, 0.4); color: #FFFFFF;
        ...
        .answer-btn:hover {{ background: rgba(6, 182, 212, 0.2); border-color: #06B6D4; transform: translateY(-3px); }}
        ...
        .start-btn {{
            background: #06B6D4; border: none; color: #FFFFFF; padding: 1.5rem 3rem; font-size: 2rem; font-weight: 900;
            border-radius: 1rem; cursor: pointer; margin-top: 3rem; align-self: center; box-shadow: 0 10px 20px rgba(6,182,212,0.4);
        }}

        /* Clue Board */
        .clue-board {{
            width: 100%; height: 100%; border: 2px solid #334155; border-radius: 10px;
            background: rgba(0,0,0,0.5); position: relative; padding: 20px;
            display: flex; flex-wrap: wrap; align-content: flex-start; gap: 10px;
        }}

        .clue-item {{
            width: 50px; height: 50px; background: rgba(6, 182, 212, 0.1); border: 2px solid #06B6D4;
            border-radius: 5px; display: flex; justify-content: center; align-items: center;
            font-size: 1.5rem; opacity: 0; transform: scale(0.5); transition: all 0.5s ease;
        }}

        .clue-item.found {{
            opacity: 1; transform: scale(1); background: rgba(6, 182, 212, 0.3);
            box-shadow: 0 0 15px #06B6D4;
        }}

        .scanner-beam {{
            position: absolute; top: 0; left: 0; width: 100%; height: 5px;
            background: #06B6D4; box-shadow: 0 0 20px 5px #06B6D4;
            opacity: 0; z-index: 5;
        }}
        ...
        .alert-box {{ background: #1E293B; padding: 3rem; border-radius: 2rem; text-align: center; border: 2px solid #06B6D4; }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">المحقق الرقمي 🕵️‍♂️</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">تشغيل الماسح الضوئي!</button>
        ...
        <div class="right-col" id="board-section" style="display: none;">
    ...
    <div class="alert-overlay" id="alert-overlay">
        <div class="alert-box">
            ...
            <button class="answer-btn" id="alert-btn" style="background: #06B6D4; border: none; color: #FFFFFF; width: 100%; justify-content: center;" onclick="nextQuestion()"></button>
        </div>
    </div>
    ...
    <script>
        ...
        function playScan() {{ playTone(1200, 2000, 'square', 0.5, 0.1); }}
        ...
        const questions = [
            {{ q: "A digital .......... investigates cyber crimes.", options: ["farmer", "detective", "builder", "singer"], correct: 1 }},
            {{ q: "If you see cyberbullying online, you should .......... it to an adult or the app.", options: ["like", "share", "report", "hide"], correct: 2 }},
            {{ q: "Taking a .......... can be used as evidence against a cyberbully.", options: ["walk", "screenshot", "nap", "shower"], correct: 1 }},
            {{ q: "We must use technology in a .......... and safe way.", options: ["bad", "dangerous", "responsible", "silly"], correct: 2 }},
            {{ q: "Always .......... the information you find online before believing it.", options: ["ignore", "delete", "verify", "color"], correct: 2 }},
            {{ q: "Digital detectives look for .......... to solve online crimes.", options: ["clues", "cars", "clothes", "chairs"], correct: 0 }},
            {{ q: "A good citizen helps to make the internet a .......... place for everyone.", options: ["scary", "worse", "better", "noisy"], correct: 2 }},
            {{ q: "You can .......... a user who is bothering you online.", options: ["thank", "invite", "block", "reward"], correct: 2 }},
            {{ q: "It's illegal to .......... someone else's work and say it's yours.", options: ["read", "share", "steal", "buy"], correct: 2 }},
            {{ q: "Remember that anything posted online can be there .......... .", options: ["for a minute", "forever", "for a day", "for an hour"], correct: 1 }}
        ];
        ...
        function revealClue() {{
            ...
            beam.style.animation = 'scan 1s linear';
            
            setTimeout(() => {{
                beam.style.animation = 'none';
                const item = document.getElementById(`clue-${{currentIndex-1}}`);
        ...
        function renderQuestion() {{
            ...
            document.getElementById('question-text').innerHTML = `<span style="color: #06B6D4;">Q${{currentIndex + 1}}:</span> ${{q.q}}`;
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playWrong();
                handleFail("انتهى الوقت! ⚠️ فقدنا الدليل!");
            }} else {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                setTimeout(() => {{
                    showAlert(true, "إجابة صحيحة! تم العثور على دليل جديد 🔍");
                }}, 800);
            }} else {{
                btns[idx].classList.add('wrong');
                btns[correctIdx].classList.add('correct');
                playWrong();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ الدليل كان مزيفاً!"), 1000);
            }}
        }}
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">🕵️‍♂️✨</div>
                <div class="question-text" style="color: white; text-align:center;">تهانينا أيها المحقق! لقد جمعت كل الأدلة وحللت القضية! 🎉</div>
...
with open(os.path.join(target_dir, "unit2_game_lesson5_6.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 5/6 for Unit 2 generated successfully.")
