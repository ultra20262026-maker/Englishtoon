...
bg_path = get_latest_image("world_portal_bg", brain_dir)
ring_path = get_latest_image("portal_ring_prop", brain_dir)
city_path = get_latest_image("future_city_bg", brain_dir)
...
city_b64 = file_to_b64(city_path)
...
<head>
    ...
    <title>الدرس السادس - بوابة المستقبل</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        body.complete {{
            background-image: url('{city_b64}');
            box-shadow: inset 0 0 200px rgba(56, 189, 248, 0.4);
        }}

        body::before {{
            content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
            background: rgba(15, 23, 42, 0.8); z-index: 1; backdrop-filter: blur(2px);
        ...
        .right-col {{
            background: rgba(15, 23, 42, 0.6); border: 2px solid rgba(56, 189, 248, 0.3); border-radius: 1rem;
        ...
        body.complete .right-col {{
            background: rgba(15, 23, 42, 0.2); border-color: rgba(56, 189, 248, 0.8);
        ...
        .portal-ring-mask {{
            ...
            border: 15px solid rgba(56, 189, 248, 0.1); box-sizing: border-box;
            transition: all 0.5s; z-index: 10; display: flex; justify-content: center; align-items: center;
        }}

        .portal-core {{
            width: 90%; height: 90%; border-radius: 50%;
            background: radial-gradient(circle, rgba(56,189,248,0.8) 0%, transparent 70%);
            opacity: 0; transition: opacity 2s ease, transform 2s ease; transform: scale(0);
            z-index: 11; mix-blend-mode: screen;
        }}
        ...
        @keyframes pulse-core {{
            ...
            100% {{ filter: brightness(1.5) drop-shadow(0 0 50px #38BDF8); }}
        }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">بوابة المستقبل 🌐</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">تفعيل البوابة!</button>
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            <div class="portal-container" id="portal-container">
                <div class="portal-ring-mask" id="ring-mask"></div>
    ...
    <script>
        ...
        function playRing() {{ playTone(600, 1200, 'sine', 0.5, 0.3); }}
        ...
        function playWin() {{
            ...
            osc.frequency.linearRampToValueAtTime(1000, audioCtx.currentTime + 2);
        ...
        const questions = [
            {{ q: "The Earth is our .......... and we must protect it.", options: ["home", "book", "car", "pen"], correct: 0 }},
            {{ q: "We can protect the environment by .......... trees.", options: ["planting", "cutting", "burning", "destroying"], correct: 0 }},
            {{ q: "The clean energy that comes from the sun is .......... energy.", options: ["solar", "water", "wind", "fossil"], correct: 0 }},
            {{ q: "Too much carbon dioxide causes .......... warming.", options: ["global", "local", "cold", "free"], correct: 0 }},
            {{ q: "We should not use .......... bags, we should use paper ones.", options: ["plastic", "gold", "silver", "diamond"], correct: 0 }},
            {{ q: "A lot of animals are in .......... because of pollution.", options: ["danger", "safety", "space", "cars"], correct: 0 }},
            {{ q: "The oceans .......... cleaned by submarines.", options: ["are", "is", "was", "were"], correct: 0 }},
            {{ q: "New trees .......... planted in our city last year.", options: ["were", "was", "is", "are"], correct: 0 }},
            {{ q: "The rubbish .......... collected every morning.", options: ["is", "are", "was", "were"], correct: 0 }},
            {{ q: "The grand trophy .......... won by our team yesterday.", options: ["was", "were", "is", "are"], correct: 0 }}
        ];
        ...
        function updatePortal() {{
            ...
            const mask = document.getElementById('ring-mask');
            
            // Create a conical gradient effect based on percentage
            mask.style.background = `conic-gradient(#38BDF8 ${{percentage}}%, transparent ${{percentage}}%)`;
            mask.style.boxShadow = `0 0 ${{percentage/2}}px rgba(56, 189, 248, 0.5)`;
            mask.style.borderColor = `rgba(56, 189, 248, ${{percentage/100}})`;

            if(currentIndex === questions.length) {{
                document.getElementById('portal-container').classList.add('active');
                setTimeout(() => {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! تم شحن جزء من البوابة ⚡");
                    }}, 1000);
                }} else {{
                    setTimeout(showWinScreen, 3000); // Wait for background reveal
                }}
            }} else {{
                btns[idx].classList.add('wrong');
                btns[correctIdx].classList.add('correct');
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ انخفضت طاقة البوابة!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                ...
                document.getElementById('ring-mask').style.background = 'none';
                document.getElementById('ring-mask').style.boxShadow = 'none';
                document.getElementById('ring-mask').style.borderColor = 'rgba(56, 189, 248, 0.1)';
            }}
            renderQuestion();
        }}
        ...
        function showWinScreen() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! لقد فتحت البوابة وعبرت للمستقبل! لقد أكملت المنهج! 🎉</div>
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">🚪</div>
                <div class="question-text" style="color: white; text-align:center;">للأسف، أغلقت البوابة وبقينا في الظلام.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit6_game_lesson6.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 6 for Unit 6 generated successfully.")
