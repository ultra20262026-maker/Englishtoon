...
bg_path = get_latest_image("dark_sea_bg", brain_dir)
submarine_path = get_latest_image("submarine_prop", brain_dir)
...
sub_b64 = file_to_b64(submarine_path)
...
<head>
    ...
    <title>الدرس الثالث - غواصة الإنقاذ</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        body.complete {{
            ...
            box-shadow: inset 0 0 200px rgba(56, 189, 248, 0.5);
        }}

        body::before {{
            content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
            background: rgba(15, 23, 42, 0.6); z-index: 1; backdrop-filter: blur(2px);
            transition: background 2s;
        }}
        
        body.complete::before {{ background: rgba(56, 189, 248, 0.2); }}
        ...
        .submarine {{
            position: absolute; left: -150px; top: 40%; width: 150px; height: 100px;
            background-image: url('{sub_b64}'); background-size: contain; background-repeat: no-repeat;
            background-position: center; z-index: 10; transition: left 1s ease-out, top 1s ease-in-out;
        ...
        .trash {{
            ...
            background-position: center; z-index: 5; opacity: 1; transition: all 0.5s ease;
        }}
        ...
        /* Random Trash Positions */
        .trash:nth-child(2) {{ top: 20%; left: 60%; }}
        .trash:nth-child(3) {{ top: 30%; left: 30%; }}
        .trash:nth-child(4) {{ top: 40%; left: 80%; }}
        .trash:nth-child(5) {{ top: 60%; left: 20%; }}
        .trash:nth-child(6) {{ top: 70%; left: 70%; }}
        .trash:nth-child(7) {{ top: 80%; left: 40%; }}
        .trash:nth-child(8) {{ top: 10%; left: 85%; }}
        .trash:nth-child(9) {{ top: 50%; left: 50%; }}
        .trash:nth-child(10) {{ top: 85%; left: 80%; }}
        .trash:nth-child(11) {{ top: 15%; left: 20%; }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">غواصة الإنقاذ 🌊</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">انطلق بالغواصة!</button>
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            <div class="ocean-container" id="ocean-container">
                <div class="submarine moving" id="submarine"></div>
                <!-- 10 Trash Bags -->
                <div class="trash" id="trash-0"></div>
                <div class="trash" id="trash-1"></div>
                <div class="trash" id="trash-2"></div>
                <div class="trash" id="trash-3"></div>
                <div class="trash" id="trash-4"></div>
                <div class="trash" id="trash-5"></div>
                <div class="trash" id="trash-6"></div>
                <div class="trash" id="trash-7"></div>
                <div class="trash" id="trash-8"></div>
                <div class="trash" id="trash-9"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const questions = [
            {{ q: "We must protect .......... from extinction.", options: ["wildlife", "cars", "buildings", "factories"], correct: 0 }},
            {{ q: "Melting ice is a result of .......... change.", options: ["climate", "history", "math", "sports"], correct: 0 }},
            {{ q: "Recycling is a .......... way to help the environment.", options: ["global", "local", "bad", "good"], correct: 3 }},
            {{ q: "Using .......... bags is better than plastic ones.", options: ["paper", "metal", "fire", "water"], correct: 0 }},
            {{ q: "Don't throw .......... in the river!", options: ["rubbish", "fish", "water", "boats"], correct: 0 }},
            {{ q: "Factories cause a lot of air .......... .", options: ["pollution", "cleaning", "health", "oxygen"], correct: 0 }},
            {{ q: "A .......... is a place where we keep rubbish.", options: ["landfill", "library", "school", "hospital"], correct: 0 }},
            {{ q: "Planting trees creates oxygen for us to .......... .", options: ["breathe", "drink", "eat", "run"], correct: 0 }},
            {{ q: "Coral .......... are beautiful habitats under the sea.", options: ["reefs", "mountains", "clouds", "deserts"], correct: 0 }},
            {{ q: "We need to save the Earth for future .......... .", options: ["generations", "planets", "aliens", "stars"], correct: 0 }}
        ];
        ...
        function updateSubmarine() {{
            const trash = document.getElementById(`trash-${{currentIndex - 1}}`);
            const sub = document.getElementById('submarine');
            
            if(trash) {{
                // Move submarine to trash
                sub.style.left = trash.offsetLeft - 100 + 'px';
                sub.style.top = trash.offsetTop - 25 + 'px';
            ...
            if(currentIndex === questions.length) {{
                setTimeout(() => {{ document.body.classList.add('complete'); }}, 1000);
            }}
        }}
        ...
        function startGame() {{
            ...
            document.getElementById('submarine').style.left = '20px';
            renderQuestion();
        }}
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playWrong();
                handleFail("انتهى الوقت! ⚠️ زادت المخلفات!");
            }} else {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! تم التقاط المخلفات 🤖");
                    }}, 1500); // Wait for sub to move
                }} else {{
                ...
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ لم تتمكن من التقاطها!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                ...
                document.querySelectorAll('.trash').forEach(t => t.classList.remove('collected'));
                document.getElementById('submarine').style.left = '20px';
                document.getElementById('submarine').style.top = '40%';
        ...
        function showWinScreen() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! لقد نظفت المحيط بالكامل! 🎉</div>
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">للأسف، تعطلت الغواصة وظل المحيط ملوثاً.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit6_game_lesson3.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 3 for Unit 6 generated successfully.")
