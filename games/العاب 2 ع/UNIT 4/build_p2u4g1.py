...
bg_path = get_latest_image("pharaoh_tomb_bg", brain_dir)
sarco_path = get_latest_image("sarcophagus_prop", brain_dir)
...
sarco_b64 = file_to_b64(sarco_path)
...
<head>
    ...
    <title>الدرس الأول - مقبرة الفرعون</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        body::before {{
            ...
            background: rgba(0, 0, 0, 0.9); z-index: 1; transition: background 1s ease;
        }}
        body.light-0::before {{ background: rgba(0, 0, 0, 0.9); }}
        body.light-1::before {{ background: rgba(0, 0, 0, 0.8); }}
        body.light-2::before {{ background: rgba(0, 0, 0, 0.7); }}
        body.light-3::before {{ background: rgba(0, 0, 0, 0.6); }}
        body.light-4::before {{ background: rgba(0, 0, 0, 0.5); }}
        body.light-5::before {{ background: rgba(0, 0, 0, 0.4); }}
        body.light-6::before {{ background: rgba(0, 0, 0, 0.3); }}
        body.light-7::before {{ background: rgba(0, 0, 0, 0.2); }}
        body.light-8::before {{ background: rgba(0, 0, 0, 0.1); }}
        body.light-9::before {{ background: rgba(0, 0, 0, 0); }}
        body.light-10::before {{ background: rgba(234, 179, 8, 0.2); }}
        ...
        .sarcophagus {{
            width: 80%; height: 60%;
            background-image: url('{sarco_b64}'); background-size: contain; background-repeat: no-repeat;
            background-position: center; transition: all 2s ease; z-index: 5;
            filter: drop-shadow(0 0 0px transparent); opacity: 0;
            mix-blend-mode: screen;
        }}
        .sarcophagus.revealed {{
            opacity: 1; filter: drop-shadow(0 0 30px rgba(234, 179, 8, 0.8));
        ...
        .torch {{
            ...
            opacity: 0; transition: opacity 1s; animation: flicker 0.1s infinite alternate;
        }}
        @keyframes flicker {{
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">مقبرة الفرعون 🏺</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">أشعل المشعل الأول!</button>
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            <div class="sarcophagus" id="sarcophagus"></div>
            <!-- Torches injected here -->
        </div>
    </div>
    ...
    <script>
        ...
        function createTorches() {{
            ...
            const positions = [
                ...
                {{ top: '70%', left: '15%' }}, {{ top: '70%', right: '15%' }},
                {{ top: '90%', left: '20%' }}, {{ top: '90%', right: '20%' }}
            ];
            ...
            positions.forEach((pos, idx) => {{
                ...
                t.className = 'torch';
                t.id = `torch-${{idx}}`;
                Object.assign(t.style, pos);
                scene.appendChild(t);
            }});
        }}

        const questions = [
            {{ q: "You must be quiet in the .......... .", options: ["stadium", "library", "park", "street"], correct: 1 }},
            {{ q: "We went to Luxor to visit the ancient .......... .", options: ["hospitals", "temples", "schools", "beaches"], correct: 1 }},
            {{ q: "The .......... is a place where you can see old things.", options: ["museum", "cinema", "club", "bank"], correct: 0 }},
            {{ q: "You .......... touch the artifacts in the museum.", options: ["must", "mustn't", "should", "can"], correct: 1 }},
            {{ q: "Tourists love to take .......... of the Pyramids.", options: ["food", "clothes", "photos", "water"], correct: 2 }},
            {{ q: "A .......... is a building from the past.", options: ["ruin", "car", "tree", "cloud"], correct: 0 }},
            {{ q: "You .......... listen to the tour guide carefully.", options: ["must", "mustn't", "shouldn't", "wouldn't"], correct: 0 }},
            {{ q: "They found an old .......... with ancient writing on it.", options: ["phone", "papyrus", "computer", "television"], correct: 1 }},
            {{ q: "Karnak is a very famous .......... in Egypt.", options: ["river", "temple", "sea", "mountain"], correct: 1 }},
            {{ q: "We can learn a lot about our .......... by visiting museums.", options: ["future", "past", "space", "tomorrow"], correct: 1 }}
        ];
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                // Light up a torch
                const torch = document.getElementById(`torch-${{lightLevel}}`);
                if(torch) torch.style.opacity = '1';
                
                lightLevel++;
                document.body.className = `light-${{lightLevel}}`;
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! اشتعلت الشعلة 🔥");
                    }}, 800);
                }} else {{
                ...
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ انطفأت شعلة!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                currentIndex = 0; lives = 3; lightLevel = 0;
                document.body.className = `light-0`;
                for(let i=0; i<10; i++) {{
                    const t = document.getElementById(`torch-${{i}}`);
                    if(t) t.style.opacity = '0';
                }}
                document.getElementById('sarcophagus').classList.remove('revealed');
            }}
            renderQuestion();
        }}

        function showWinScreen() {{
            const sarco = document.getElementById('sarcophagus');
            sarco.classList.add('revealed');
            
            const qBox = document.getElementById('question-box');
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">🏺✨</div>
                <div class="question-text" style="color: white; text-align:center;">تهانينا! لقد أنرت المقبرة بالكامل واكتشفت التابوت السحري! 🎉</div>
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">🌑</div>
                <div class="question-text" style="color: white; text-align:center;">للأسف، خيم الظلام على المقبرة.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit4_game_lesson1.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 1 for Unit 4 generated successfully.")
