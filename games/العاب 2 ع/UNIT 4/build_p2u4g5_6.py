...
bg_path = get_latest_image("papyrus_room_bg", brain_dir)
papyrus_path = get_latest_image("blank_papyrus_prop", brain_dir)
...
papyrus_b64 = file_to_b64(papyrus_path)
...
<head>
    ...
    <title>الدرس 5 و 6 - البردي السحري</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        .answer-btn:hover {{ background: rgba(168, 85, 247, 0.2); border-color: #C084FC; transform: translateY(-3px); }}
        ...
        .start-btn {{
            background: #A855F7; border: none; color: #FFFFFF; padding: 1.5rem 3rem; font-size: 2rem; font-weight: 900;
            border-radius: 1rem; cursor: pointer; margin-top: 3rem; align-self: center; box-shadow: 0 10px 20px rgba(168,85,247,0.4);
        }}

        /* Papyrus Scene */
        .papyrus {{
            width: 90%; height: 80%; background-image: url('{papyrus_b64}');
        ...
        .magic-text {{
            color: transparent; font-family: 'Courier New', Courier, monospace; font-size: 1.2rem; font-weight: bold;
            text-align: center; width: 60%; line-height: 2; text-shadow: 0 0 10px rgba(255,255,255,0);
            transition: color 1s, text-shadow 1s; word-wrap: break-word;
        }}

        .magic-text.revealed {{
            color: #C084FC; text-shadow: 0 0 5px #A855F7, 0 0 15px #D8B4FE;
        }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">البردي السحري 📜</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">اكتب التاريخ!</button>
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            <div class="papyrus" id="papyrus">
                <div class="magic-text" id="t-0">ANCIENT SECRETS</div>
                <div class="magic-text" id="t-1">𓀀 𓀁 𓀂 𓀃</div>
                <div class="magic-text" id="t-2">THE PHARAOH</div>
                <div class="magic-text" id="t-3">𓀄 𓀅 𓀆 𓀇</div>
                <div class="magic-text" id="t-4">BUILT THE</div>
                <div class="magic-text" id="t-5">𓀈 𓀉 𓀊 𓀋</div>
                <div class="magic-text" id="t-6">GREAT TEMPLES</div>
                <div class="magic-text" id="t-7">𓀌 𓀍 𓀎 𓀏</div>
                <div class="magic-text" id="t-8">TO LAST FOREVER</div>
                <div class="magic-text" id="t-9">𓀐 𓀑 𓀒 𓀓</div>
            </div>
        </div>
    </div>
    ...
    <div class="alert-overlay" id="alert-overlay">
        <div class="alert-box">
            ...
            <button class="answer-btn" id="alert-btn" style="background: #A855F7; border: none; color: #FFFFFF; width: 100%; justify-content: center;" onclick="nextQuestion()"></button>
        </div>
    </div>
    ...
    <script>
        ...
        const questions = [
            {{ q: "Ancient Egyptians wrote on .......... .", options: ["paper", "papyrus", "plastic", "glass"], correct: 1 }},
            {{ q: "We can see beautiful .......... inside the temple.", options: ["statues", "cars", "trains", "planes"], correct: 0 }},
            {{ q: "The Valley of the .......... is in Luxor.", options: ["Queens", "Kings", "Princes", "Knights"], correct: 1 }},
            {{ q: "The tour .......... told us interesting stories about the ruins.", options: ["driver", "pilot", "guide", "doctor"], correct: 2 }},
            {{ q: "Let's .......... a visit to the museum.", options: ["do", "plan", "play", "eat"], correct: 1 }},
            {{ q: "Luxor is a very .......... city with many tourists.", options: ["quiet", "empty", "busy", "ugly"], correct: 2 }},
            {{ q: "An .......... is a person who studies old things from the ground.", options: ["engineer", "archaeologist", "farmer", "vet"], correct: 1 }},
            {{ q: "A .......... is a strong building from the past to protect people.", options: ["castle", "house", "tent", "flat"], correct: 0 }},
            {{ q: "Did you .......... the ancient ruins?", options: ["visited", "visiting", "visit", "visits"], correct: 2 }},
            {{ q: "Historic places are very .......... to our country's tourism.", options: ["bad", "important", "boring", "lazy"], correct: 1 }}
        ];
        ...
        function updatePapyrus() {{
            const t = document.getElementById(`t-${{currentIndex-1}}`);
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playWrong();
                handleFail("انتهى الوقت! ⚠️ توقف القلم السحري!");
            }} else {{
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                if(currentIndex < questions.length) {{
                    setTimeout(() => {{
                        showAlert(true, "إجابة صحيحة! نُقش السطر 📜");
                    }}, 1000);
                }} else {{
                ...
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ تلاشت الكتابة!"), 1000);
            }}
        }}
        ...
        function showWinScreen() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! اكتملت لفافة البردي السحرية وعُرفت الأسرار! 🎉</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
            document.getElementById('papyrus').style.filter = 'drop-shadow(0 0 40px #A855F7)';
            playWin();
        }}
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                <div style="font-size: 6rem; text-align:center; margin-bottom: 1rem;">🔥</div>
                <div class="question-text" style="color: white; text-align:center;">للأسف، احترقت كلمات البردي.</div>
                <button class="start-btn" style="width: 100%;" onclick="location.reload()">العب مرة أخرى 🔄</button>
            `;
        }}
    </script>
...
with open(os.path.join(target_dir, "unit4_game_lesson5_6.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 5 & 6 for Unit 4 generated successfully.")
