...
bg_path = get_latest_image("race_track_bg", brain_dir)
runner_path = get_latest_image("hurdle_runner_prop", brain_dir)
...
runner_b64 = file_to_b64(runner_path)
...
<head>
    ...
    <title>الدرس الثاني - سباق الحواجز</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        .runner {{
            position: absolute; bottom: 10%; right: 10%; width: 120px; height: 120px;
            background-image: url('{runner_b64}'); background-size: contain; background-repeat: no-repeat;
            background-position: center; transition: all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); z-index: 5;
            filter: drop-shadow(0 5px 10px rgba(0,0,0,0.5));
            mix-blend-mode: screen;
        }}

        .hurdle {{
            position: absolute; bottom: 10%; right: 40%; width: 50px; height: 60px;
            border: 4px solid #EF4444; border-bottom: none; border-radius: 5px 5px 0 0;
            background: rgba(239,68,68,0.2); transition: right 0.5s ease;
        }}

        .finish-line {{
            position: absolute; bottom: 10%; right: 90%; width: 10px; height: 100px;
            background: repeating-linear-gradient(45deg, white, white 10px, black 10px, black 20px);
            opacity: 0; transition: opacity 1s;
        }}

        @keyframes jump {{
            0% {{ bottom: 10%; transform: rotate(0deg); }}
            50% {{ bottom: 35%; transform: rotate(-10deg); }}
            100% {{ bottom: 10%; transform: rotate(0deg); }}
        }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">سباق الحواجز 🏃‍♂️</h1>
                <div class="stats">
            ...
            <button class="start-btn" id="start-btn" onclick="startGame()">ابدأ السباق!</button>
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            <div class="track">
                ...
                <div class="hurdle" id="hurdle"></div>
    ...
    <script>
        ...
        function playJump() {{ playTone(300, 800, 'sine', 0.5, 0.4); }}
        ...
        const questions = [
            {{ q: "She is going to .......... in the next race.", options: ["run", "running", "runs", "ran"], correct: 0 }},
            {{ q: "A hurdle is a type of .......... in a race.", options: ["food", "obstacle", "car", "drink"], correct: 1 }},
            {{ q: "What are you going to .......... when you finish school?", options: ["doing", "did", "do", "does"], correct: 2 }},
            {{ q: "To succeed, we must .......... our fears.", options: ["sleep", "overcome", "ignore", "eat"], correct: 1 }},
            {{ q: "Next year, I .......... learn a new language.", options: ["going to", "am going to", "went", "is going"], correct: 1 }},
            {{ q: "The runner jumped over the .......... very quickly.", options: ["river", "hurdle", "mountain", "sky"], correct: 1 }},
            {{ q: "We must be brave to face new .......... .", options: ["colors", "games", "challenges", "toys"], correct: 2 }},
            {{ q: "They aren't .......... to travel this summer.", options: ["go", "going", "goes", "went"], correct: 1 }},
            {{ q: "Is he .......... to win the competition?", options: ["going", "will", "shall", "doing"], correct: 0 }},
            {{ q: "Practice hard, and you .......... achieve your goals.", options: ["will", "going to", "goes", "won't"], correct: 0 }}
        ];
        ...
        let runProgress = 10; // start at right 10%

        function updatePositions() {{
            const player = document.getElementById('player');
            const hurdle = document.getElementById('hurdle');
            
            player.style.right = `${{runProgress}}%`;
            hurdle.style.right = `${{runProgress + 30}}%`; // hurdle is always ahead
            
            if(currentIndex >= 9) {{
                document.getElementById('finish-line').style.opacity = '1';
                hurdle.style.display = 'none'; // No hurdle on last step
            }}
        }}
        ...
        function updateTimer() {{
            ...
            if (timeRemaining <= 0) {{
                ...
                playHit();
                handleFail("انتهى الوقت! ⚠️ اصطدمت بالحاجز!");
            }} else {{
        ...
        function handleFail(msg) {{
            ...
            const player = document.getElementById('player');
            player.style.filter = 'grayscale(1) drop-shadow(0 0 10px red)';
            setTimeout(() => player.style.filter = 'drop-shadow(0 5px 10px rgba(0,0,0,0.5))', 1000);
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                // Jump animation
                player.style.animation = 'jump 0.8s ease';
                playJump();
                
                setTimeout(() => {{
                    player.style.animation = 'none';
                    updatePositions();
                    showAlert(true, "إجابة صحيحة! قفزة ناجحة 🏃‍♂️");
                }}, 800);
            }} else {{
                btns[idx].classList.add('wrong');
                btns[correctIdx].classList.add('correct');
                playHit();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ اصطدمت بالحاجز!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                currentIndex = 0; lives = 3; runProgress = 10;
                document.getElementById('hurdle').style.display = 'block';
                updatePositions();
            }}
            renderQuestion();
        }}
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! لقد تجاوزت جميع الحواجز ووصلت إلى خط النهاية بنجاح! 🎉</div>
...
with open(os.path.join(target_dir, "unit3_game_lesson2.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 2 for Unit 3 generated successfully.")
