...
bg_path = get_latest_image("future_city_base_bg", brain_dir)
tower_path = get_latest_image("digital_tower_prop", brain_dir)
...
tower_b64 = file_to_b64(tower_path)
...
<head>
    ...
    <title>الدرس الثالث - باني المستقبل</title>
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', 'Cairo', sans-serif; }}
        ...
        .right-col {{
            background: rgba(15, 23, 42, 0.6); border: 2px solid rgba(59, 130, 246, 0.4); border-radius: 1rem;
            height: 80vh; position: relative; box-shadow: inset 0 0 40px rgba(59, 130, 246, 0.2);
        ...
        .tower-block {{
            width: 150px; height: 50px; margin-top: -10px;
            background-image: url('{tower_b64}'); background-size: cover; background-position: center;
            opacity: 0; transform: translateY(-50px) scale(1.2); transition: all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);
            z-index: 5; mix-blend-mode: screen; filter: drop-shadow(0 0 10px #3B82F6);
        }}
        ...
        .crane {{
            position: absolute; top: 0; right: -50px; width: 100px; height: 10px;
            background: #EAB308; border: 2px solid #CA8A04; transform-origin: left center;
            animation: swing 4s ease-in-out infinite alternate; z-index: 4;
        }}

        .crane-line {{
            position: absolute; top: 10px; right: 10px; width: 2px; height: 100px;
            background: #94A3B8;
        }}
...
<body>
    ...
    <div class="layout-container">
        <div class="left-col">
            <div class="header" id="game-header" style="display: none;">
                <h1 class="game-title">باني المستقبل 🏗️</h1>
                <div class="stats">
        ...
        <div class="right-col" id="game-scene" style="display: none;">
            <div class="crane"><div class="crane-line"></div></div>
    ...
    <script>
        ...
        function playBuild() {{ playTone(300, 600, 'square', 0.5, 0.3); }}
        function playDrop() {{ playTone(150, 50, 'sawtooth', 0.5, 0.5); }}
        ...
        const questions = [
            {{ q: "I think cars .......... fly in the future.", options: ["will", "are going to", "goes", "doing"], correct: 0 }},
            {{ q: "Look at those dark clouds! It .......... rain.", options: ["will", "is going to", "go", "going"], correct: 1 }},
            {{ q: "They .......... build a new hospital next year.", options: ["are going to", "will going to", "going to", "go"], correct: 0 }},
            {{ q: "We must plan well to build a good .......... .", options: ["past", "history", "future", "yesterday"], correct: 2 }},
            {{ q: "Maybe scientists .......... find a cure for this disease.", options: ["going", "will", "are going", "go"], correct: 1 }},
            {{ q: "She is studying hard because she .......... be a doctor.", options: ["will", "going", "is going to", "go to"], correct: 2 }},
            {{ q: "The architect is drawing the .......... for the new bridge.", options: ["food", "water", "plans", "cars"], correct: 2 }},
            {{ q: "I .......... help you carry those heavy boxes.", options: ["am going", "will", "going", "goes"], correct: 1 }},
            {{ q: "What .......... you going to do this weekend?", options: ["is", "am", "are", "will"], correct: 2 }},
            {{ q: "Building a tower requires a lot of .......... .", options: ["sleeping", "eating", "effort", "playing"], correct: 2 }}
        ];
        ...
        function initTower() {{
            ...
            // Create blocks top-down so they stack correctly (DOM order)
            for(let i=9; i>=0; i--) {{
                const block = document.createElement('div');
                block.className = 'tower-block';
                block.id = `block-${{i}}`;
                block.style.zIndex = 10 - i;
        ...
        function selectOption(idx) {{
            ...
            if (idx === correctIdx) {{
                ...
                // Build animation
                const block = document.getElementById(`block-${{currentIndex}}`);
                if(block) block.classList.add('built');
                
                currentIndex++;
                playBuild();
                
                setTimeout(() => {{
                    showAlert(true, "إجابة صحيحة! تم بناء طابق جديد 🏗️");
                }}, 800);
            }} else {{
                btns[idx].classList.add('wrong');
                btns[correctIdx].classList.add('correct');
                playWrong();
                setTimeout(() => handleFail("إجابة خاطئة! ❌ انهار الجزء!"), 1000);
            }}
        }}
        ...
        function nextQuestion() {{
            ...
            if (lives <= 0) {{ 
                ...
                for(let i=0; i<10; i++) {{
                    ...
                    if(block) block.classList.remove('built');
                }}
            }}
            renderQuestion();
        }}
        ...
        function showGameOver() {{
            ...
            qBox.innerHTML = `
                ...
                <div class="question-text" style="color: white; text-align:center;">تهانينا! لقد قمت ببناء برج المستقبل بنجاح! 🎉</div>
...
with open(os.path.join(target_dir, "unit3_game_lesson3.html"), "w", encoding="utf-8") as f:
    f.write(html_content)

print("Game 3 for Unit 3 generated successfully.")
